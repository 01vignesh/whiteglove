...
function upload_admin_image(array $file): string
{
    ...
    $fileName = 'admin_' . bin2hex(random_bytes(8)) . '.' . $allowed[$mime];
    ...
    return '/WhiteGlove/public/uploads/admins/' . $fileName;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $currentStmt = $pdo->prepare(
            'SELECT profile_image_url, security_answer_hash
             FROM users
             WHERE id = ?
             LIMIT 1'
        );
        ...
        if ($hasUploadedImage) {
            $profileImageUrl = upload_admin_image($_FILES['profile_image_file']);
        }
...
?>
<?php render_admin_module_page_start('Admin Profile'); ?>
    <section class="provider-hero">
        <h1 class="h4 mb-1">Admin Profile</h1>
        <p class="mb-0">Update admin account details, profile image, and security question settings.</p>
    </section>
    ...
    <section class="provider-card">
        <?php if ((string) ($profile['profile_image_url'] ?? '') !== ''): ?>
            <div class="provider-avatar-wrap">
                <img class="provider-avatar" src="<?php echo htmlspecialchars((string) $profile['profile_image_url'], ENT_QUOTES, 'UTF-8'); ?>" alt="Admin profile picture">
            </div>
        <?php endif; ?>
